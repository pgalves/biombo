...
void KGpgMe::init(gpgme_protocol_t proto)
{
    ...
    if (err) {
        KMessageBox::error(kapp->activeWindow(), QString("init: %1: %2")
...
bool KGpgMe::decrypt(const QByteArray& inBuffer, QCA::SecureArray* outBuffer)
{
    gpgme_error_t err = 0;
    gpgme_data_t in = 0, out = 0;
            ...
            if (!err) {
                ...
                if (!err) {
                    ...
                    if (result->unsupported_algorithm) {
                        ...
                        err = readToBufferSec(out, outBuffer);
                    }
                }
            }
        }
...
gpgme_error_t KGpgMe::readToBufferSec(gpgme_data_t in, QCA::SecureArray* outBuffer) const
{
    int ret;
    gpgme_error_t err = GPG_ERR_NO_ERROR;
